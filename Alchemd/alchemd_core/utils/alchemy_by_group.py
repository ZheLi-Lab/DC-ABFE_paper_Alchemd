...
class DecoupleGroups():
    """
    A class for managing atomic groups in staged divide-and-conquer alchemical transformations.

    This class handles the classification of atoms within an alchemical region during
    multi-stage free energy calculations. It organizes atoms into three categories:
    already decoupled atoms, atoms to be decoupled in the current stage, and
    remaining atoms.

    Parameters
    ----------
    groups : dict
        A dictionary defining atom groups for staged transformation. Keys should be
        strings starting with 'group' (e.g., 'group1', 'group2'), and values should
        be sets of atom indices (starting from 0).

    Returns
    ----------
    groups : dict
        The input dictionary of atom groups
    all_atoms : set
        Union of all atom sets across all groups
    already_decoupled_atoms : set
        Atoms that have been decoupled in previous stages
    tobe_decoupled_atoms : set
        Atoms to be decoupled in the current stage
    remaining_atoms : set
        Atoms that will be decoupled in future stages

    Examples
    --------
    >>> groups = {'group1': {0,1,2}, 'group2': {3,4,5}}
    >>> decouple = DecoupleGroups(groups)
    >>> # Get atoms for stage 1
    >>> already, current, remaining = decouple.get_group(1)
    >>> print(current)  # Atoms to be decoupled in stage 1
    {0, 1, 2}
    """
    def __init__(self, groups:dict):
        ...
        for i in self.groups:
            self.all_atoms=self.all_atoms|self.groups[i]

        self.already_decoupled_atoms=set()
        self.tobe_decoupled_atoms=set()
        self.remaining_atoms=set()
    def get_group(self, current_group:int):
        # return already_decoupled_atoms; tobe_decoupled_atoms; remaining_atoms
        if current_group == 0:
            self.remaining_atoms = self.all_atoms
        elif current_group > 0:
            for i in range(current_group-1):
                group_num=i+1
                group_key='group'+str(group_num)
                self.already_decoupled_atoms=self.already_decoupled_atoms|self.groups[group_key]
            self.tobe_decoupled_atoms=self.groups['group'+str(current_group)]
            self.remaining_atoms=self.all_atoms-self.already_decoupled_atoms-self.tobe_decoupled_atoms
        else:
            print ("nb_group can't be set to a negative integer")
        return self.already_decoupled_atoms, self.tobe_decoupled_atoms, self.remaining_atoms
...
class AlchemicalState(states.GlobalParameterState):
    ...
    lambda_electrostatics_env = _LambdaParameter('lambda_electrostatics_env')
    lambda_electrostatics_chgprod_square = _LambdaParameter('lambda_electrostatics_chgprod_square')
...
_ALCHEMICAL_REGION_ARGS = collections.OrderedDict([
    ...
    ('softcore_alpha', 0.2), ('softcore_a', 1), ('softcore_b', 1), ('softcore_c', 6),
...
class AbsoluteAlchemicalFactory(object):
    ...
    Creating the an alchemical factory for the DC-ABFE calculation
    ...
    >>> system = prmtop.createSystem(constraints=app.HBonds, nonbondedMethod=app.PME, rigidWater=True, nonbondedCutoff=9.0*unit.angstroms, hydrogenMass=None)
    >>> thermodynamic_state = states.ThermodynamicState(system, 298*unit.kelvin)
    >>> topology = prmtop.toplogy
    >>> mdtraj_topology = md.Topology.from_openmm(topology)
    >>> alchem_group = 'resname MOL' # the selection string for the mdtraj_topology
    >>> alchem_atoms = mdtraj_topology.select(alchem_group)
    >>> alchemical_region = AlchemicalRegion(alchemical_atoms=alchem_atoms,)
    ...
    >>> alchemical_system = factory.create_alchemical_system(self.thermodynamic_state.system, alchemical_region,
                            ...
                            set_rbfe_exception=False) # details of set_initial_charge, set_target_charge, decouple_groups_nb and current_group_nb could be seen in the doc string of create_alchemical_system.
    ...
    def create_alchemical_system(self, reference_system, alchemical_regions,
        ...
        decouple_groups_nb : decouple by groups, False means do not decouple by group. 
            If need to turn on, pass dict prepared like the following one,
                nb_groups={
                    'group1':{5235,5236},
                    'group2':{5237,5238},
                    'group3':{5239,5240,5241,5242},
                    'group4':{....},
                    ...
                }
                Note that the atom id starts from 0
        current_group_nb: int
            If parameter decouple_groups_nb = nb_groups, as described above,
            current_group_nb = 1 means decouple the atoms defined in 'group1', groups 2-4 will remain unchanged
            current_group_nb = 2 means decouple the atoms defined in 'group2', group1 has already decoupled, and group 3-4 will remain unchanged
        set_initial_charge: False or dict
            if set to False, initial charge of the simulation system will be used.
            if a dict is passed to this parameter, will set customized initial charge for the alchemical region.
            Example: set_initial_charge=ini_chg, ini_chg looks like:
                ini_chg={
                    5235: 0.23,
                    5236:-0.15,
                    5237: 0.10,
                ...
                Note that the atom id starts from 0
        set_target_charge: False or dict
            if set to False, 0 will be used.
            if a dict is passed to this parameter, will set customized target charge for the alchemical region.
            Example: set_target_charge=tgt_chg, tgt_chg looks like:
                ini_chg={
                    5235: 0.13,
                    5236:-0.05,
                    5237: 0.02,
        ...
        set_rbfe_exception: bool, optional
            To determine if set the combined-structure based relative binding free energy setting.
            By default, it is False.
            If it is True, the property self.set_rbfe_exception will be set to True.
            This option will use the DecoupleGroups(self.decouple_groups_nb)'s
        ...
        self.set_initial_charge=set_initial_charge
        self.set_target_charge=set_target_charge

        if hasattr(self, 'alchem_settings'):
            self.set_rbfe_exception = self.alchem_settings.set_rbfe_exception  # added by ylzhong
        else:
            self.set_rbfe_exception = False
    ...
    def _get_sterics_energy_expressions(self, lambda_variable_suffixes, mode):
        ...
        sterics_energy_expression = 'U_sterics;'

        print(f'Using softcore: {mode}')



        if mode == 'gapsys':
            sterics_energy_expression += "U_sterics = select(step(r - r_LJ), 4*epsilon_lambda*x*(x-1.0), U_sterics_quad);"

            #sterics_energy_expression += "U_sterics_quad = Force*(((r - r_LJ)^2)/2 - (r - r_LJ)) + U_sterics_cut;"
            #sterics_energy_expression += "U_sterics_cut = 4*epsilon_lambda*((sigma/r_LJ)^6)*(((sigma/r_LJ)^6) - 1.0);"
            #sterics_energy_expression += "Force = -4*epsilon_lambda*((-12*sigma^12)/(r_LJ^13) + (6*sigma^6)/(r_LJ^7));"

            sterics_energy_expression += "U_sterics_quad = 4*epsilon_lambda*(Func_a+Func_b+Func_c);"
            sterics_energy_expression += "Func_a = ((78*sigma^12)/(r_LJ^14)-(21*sigma^6)/(r_LJ^8))*r^2;"
            sterics_energy_expression += "Func_b = -((168*sigma^12)/(r_LJ^13)-(48*sigma^6)/(r_LJ^7))*r;"
            sterics_energy_expression += "Func_c = (91*sigma^12)/(r_LJ^12)-(28*sigma^6)/(r_LJ^6);"

            #sterics_energy_expression += "U_sterics_quad = 4*epsilon_lambda*(Fa+Fb+Fc+Fd);"
            #sterics_energy_expression += "Fa = ((12*sigma^12)/(r_LJ^13)-(6*sigma^6)/(r_LJ^7))*(r^2)/2;"
            #sterics_energy_expression += "Fb = -((12*sigma^12)/(r_LJ^12)-(6*sigma^6)/(r_LJ^6))*r;"
            #sterics_energy_expression += "Fc = ((12*sigma^12)/(r_LJ^13)-(6*sigma^6)/(r_LJ^7))*((r_LJ^2)/2-r_LJ);"
            #sterics_energy_expression += "Fd = ((sigma/r_LJ)^6)*(((sigma/r_LJ)^6) - 1.0);"

            sterics_energy_expression += "x = (sigma/r)^6;"
            sterics_energy_expression += "r_LJ = softcore_alpha*((26/7)*(sigma^6)*(1-lambda_value)+(1e-12)*step(lambda_value-1))^(1/6);"
            # We must add a small number 0.01*step({0}-1) for lambda=0, otherwise will raise an error. Could be an error due to the openmm compliation. Double check this!
        else:
            sterics_energy_expression += "U_sterics = 4*epsilon_lambda*x*(x-1.0); x = (sigma/reff_sterics)^6;"
            sterics_energy_expression += "reff_sterics = sigma*((softcore_alpha*(1.0-lambda_value) + (r/sigma)^6))^(1/6);"

        sterics_energy_expression += "epsilon_lambda = epsilon*(lambda_value);"

        if mode == 'ssc2':
            #sterics_energy_expression += "lambda_value = -6*({0}-1)^5 - 15*({0}-1)^4 - 10*({0}-1)^3;".format(lambda_variable_name)
            sterics_energy_expression += "lambda_value = 6*({0})^5 - 15*({0})^4 + 10*({0})^3;".format(lambda_variable_name)
        else:
            sterics_energy_expression += "lambda_value = ({0});".format(lambda_variable_name)


        sterics_mixing_rules = ('epsilon = sqrt(epsilon1*epsilon2);'  # Mixing rule for epsilon.
                                'sigma = 0.5*(sigma1 + sigma2);')  # Mixing rule for sigma.

        return sterics_mixing_rules, sterics_energy_expression
    ...
    def _alchemically_modify_NonbondedForce(self, reference_force, alchemical_regions, alchemical_regions_interactions):
        ...
        # added by zli
        #def get_already_decoupled_alchemical_atomset():
        #    return {5235,5236,5237,5238}
        #def get_tobe_decoupled_alchemical_atomset():
        #    return {5239,5240,5241,5242}
        if not self.decouple_groups_nb:
            already_decoupled_alchemical_atomset = set()
            tobe_decoupled_alchemical_atomset = all_alchemical_atoms
            remaining_alchemical_atomset = set()
        else:
            decouple_groups_nb=DecoupleGroups(self.decouple_groups_nb)
            already_decoupled_alchemical_atomset, tobe_decoupled_alchemical_atomset, remaining_alchemical_atomset = decouple_groups_nb.get_group(self.current_group_nb)
            if decouple_groups_nb.all_atoms != all_alchemical_atoms:
                print(decouple_groups_nb.all_atoms)
                # The origin version of here is 'self.decouple_groups_nb.all_atoms', which is a bug. ylzhong.
                # Reasons:
                #   type self.decouple_groups_nb is a dict
                #   type decouple_groups_nb is a DecoupleGroups
                print(all_alchemical_atoms)
                raise ValueError('alchemical atoms are not the same with that defined by group')
        #already_decoupled_alchemical_atomset = get_already_decoupled_alchemical_atomset()
        #tobe_decoupled_alchemical_atomset = get_tobe_decoupled_alchemical_atomset()
        #remaining_alchemical_atomset = all_alchemical_atoms - tobe_decoupled_alchemical_atomset - already_decoupled_alchemical_atomset
        ...
        # Add exception between alchemical atoms
        if self.set_rbfe_exception: # added by ylzhong
            unit_A_atomset, unit_B_atomset, common_atomset = self.alchem_settings.atomsets

            # Use to collect atomset of units.
            #   Group 1 is the group of unit A,
            #   Group 2 is the group of unit B,
            #   Group 3 is the group of common atoms
            for atomA in unit_A_atomset:
                for atomB in unit_B_atomset:
                    nonbonded_force.addException(atomA, atomB, 0.0, 1.0, 0.0, True)
        ...
        if use_exact_pme_treatment:
            ...
            for alchemical_region in alchemical_regions:
                if alchemical_region.name is None:
                    ...
                    nonbonded_force.addGlobalParameter('lambda_electrostatics_env', 1.0)
                    nonbonded_force.addGlobalParameter('lambda_electrostatics_chgprod_square', 1.0)
                else:
                    nonbonded_force.addGlobalParameter(f'lambda_electrostatics_{alchemical_region.name}', 1.0)
                    nonbonded_force.addGlobalParameter(f'lambda_electrostatics_chgprod_square_{alchemical_region.name}', 1.0) # need to make sure that how the f'lambda_electrostatics_chgprod_square_{alchemical_region.name}' be updated by the external lambdas.json.
                    nonbonded_force.addGlobalParameter('lambda_electrostatics_env', 1.0)

            if not alchemical_region.annihilate_electrostatics: #decouple electrostacs. We need to add exception between alchemical/non-alchemical atoms and rebuild them.
                for atom1 in all_alchemical_atoms:
                    for atom2 in nonalchemical_atomset:
                        nonbonded_force.addException(atom1, atom2, 0.0, 1.0, 0.0, True)
        ...
        for alchemical_regions_pairs in single_regions+pair_regions:
            ...
            # *****added by zli: ↑ This term is replaced by the following three terms ↓                        |
            # --------------------------------------------------------------------------------------------------
            # naa_sterics_custom_nonbonded_force       | s_interactions non-alchem/alchem-already-decoupled    |
            # --------------------------------------------------------------------------------------------------
            # nat_sterics_custom_nonbonded_force       | s_interactions non-alchem/alchem-to-be-decoupled      |
            # --------------------------------------------------------------------------------------------------
            # nar_sterics_custom_nonbonded_force       | s_interactions non-alchem/alchem-remaining            |
            #                                          | (Do not decouple for this calculation)                |
            # --------------------------------------------------------------------------------------------------
            # --------------------------------------------------------------------------------------------------
            ...
            # *****added by zli: ↑ This term is replaced by the following three terms ↓                        |
            # --------------------------------------------------------------------------------------------------
            # naa_sterics_custom_bond_force            | s_exceptions non-alchem/alchem-already-decoupled      |
            # --------------------------------------------------------------------------------------------------
            # nat_sterics_custom_bond_force            | s_exceptions non-alchem/alchem-to-be-decoupled        |
            # --------------------------------------------------------------------------------------------------
            # nar_sterics_custom_bond_force            | s_exceptions non-alchem/alchem-remaining              |
            #                                          | (Do not decouple for this calculation)                |
            # --------------------------------------------------------------------------------------------------
            # --------------------------------------------------------------------------------------------------
            ...
            # added by zli: fixed_lambda parameter
            def create_force(force_cls, energy_expression, lambda_variable_name, lambda_var_suffixes, is_lambda_controlled, fixed_lambda = 1.0):
                ...
                if is_lambda_controlled:
                    ...
                    for suffix in lambda_var_suffixes:
                        name = (lambda_variable_name + suffix)
                        energy_expression = energy_expression + name + '=' + str(fixed_lambda) + ';'
                    force = force_cls(energy_expression)
                return force
...
#                naa_sterics_custom_nonbonded_force = create_force(openmm.CustomNonbondedForce, sterics_energy_expression,
#                                                                 'lambda_sterics', lambda_var_suffixes, is_lambda_controlled=False, fixed_lambda=0.0)
                nat_sterics_custom_nonbonded_force = create_force(openmm.CustomNonbondedForce, sterics_energy_expression,
                                                                 'lambda_sterics', lambda_var_suffixes, is_lambda_controlled=True)
                nar_sterics_custom_nonbonded_force = create_force(openmm.CustomNonbondedForce, sterics_energy_expression,
                                                                 'lambda_sterics', lambda_var_suffixes, is_lambda_controlled=False, fixed_lambda=1.0)
                aa_sterics_custom_nonbonded_force = create_force(openmm.CustomNonbondedForce, sterics_energy_expression,
                                                                 'lambda_sterics', lambda_var_suffixes, alchemical_region.annihilate_sterics)
                all_sterics_custom_nonbonded_forces = [ #naa_sterics_custom_nonbonded_force, 
                                                        nat_sterics_custom_nonbonded_force,
                                                        nar_sterics_custom_nonbonded_force,
...
#                                                        aa_sterics_custom_nonbonded_force ]
            ...
            if use_exact_pme_treatment:
                if alchemical_region.annihilate_electrostatics:
                    ...
                    # decouple electrostatics. The short-range columb interaction between alchemical and non-alchemical has been excluded, and now we rebuild them.
                    # TODO: add custom electrostatic force as described by https://github.com/choderalab/openmmtools/issues/376
                    # expression: (lambda^2)*charge1*charge2 + (lambda-lambda^2)*(charge1*tgt_charge2+tgt_charge1*charge2) + (1-lambda)^2*tgt_charge1*tgt_charge2
                    # TODO: make this compatable with create_force 
                    lig_electrostatics_addback_energy_expression = 'ONE_4PI_EPS0*chargeprod_eff/r;'
                    lig_electrostatics_addback_energy_expression += 'ONE_4PI_EPS0 = %.16e;' % (ONE_4PI_EPS0)
                    lig_electrostatics_addback_energy_expression += 'chargeprod_eff = ({0})^2*chargeprod+(({0})-({0})^2)*cross_chargeprod+(1-({0}))^2*tgt_chargeprod;'.format(
                        'lambda_electrostatics')
                    lig_electrostatics_addback_energy_expression += 'chargeprod = charge1*charge2; cross_chargeprod = charge1*tgt_charge2+tgt_charge1*charge2; '
                    lig_electrostatics_addback_energy_expression += 'tgt_chargeprod = tgt_charge1 * tgt_charge2'

                    na_electrostatics_custom_nonbonded_force = openmm.CustomNonbondedForce(
                        lig_electrostatics_addback_energy_expression)
            ...
            for force in all_electrostatics_custom_nonbonded_forces:
                # TODO: I'm not sure using 'if' here is a good idea.
                if alchemical_region.annihilate_electrostatics:
                    # This else will not be used for PME treatment
                    force.addPerParticleParameter("charge")  # partial charge
                    force.addPerParticleParameter("sigma")  # Lennard-Jones sigma
                else:
                    # decouple electrostatics. We need two parameters for each atom.
                    force.addPerParticleParameter("charge") # initial partial charge
                    force.addPerParticleParameter("tgt_charge") # target partial charge
            ...
            ##changed by zli: changed sterics_custom_bond_force related terms
            if len(lambda_var_suffixes) > 1:
                ...
                #                                            'lambda_sterics', lambda_var_suffixes, is_lambda_controlled=True)
                naa_sterics_custom_bond_force = create_force(openmm.CustomBondForce, exceptions_sterics_energy_expression,
                                                                 'lambda_sterics', lambda_var_suffixes, is_lambda_controlled=False, fixed_lambda=0.0)
                nat_sterics_custom_bond_force = create_force(openmm.CustomBondForce, exceptions_sterics_energy_expression,
                                                                 'lambda_sterics', lambda_var_suffixes, is_lambda_controlled=True)
                nar_sterics_custom_bond_force = create_force(openmm.CustomBondForce, exceptions_sterics_energy_expression,
                                                                 'lambda_sterics', lambda_var_suffixes, is_lambda_controlled=False, fixed_lambda=1.0)
                aa_sterics_custom_bond_force = create_force(openmm.CustomBondForce, exceptions_sterics_energy_expression,
                                                                 'lambda_sterics', lambda_var_suffixes, alchemical_region.annihilate_sterics)
                all_sterics_custom_bond_forces = [ naa_sterics_custom_bond_force, 
                                                        nat_sterics_custom_bond_force,
                                                        nar_sterics_custom_bond_force,
            ...
            for particle_index in range(nonbonded_force.getNumParticles()):
                ...
                # Set initial and target charge
                if self.set_initial_charge is not False:
                    try: # May exist the situation that particle_index out of the mol.pdbx atom_index, in this situation, use the original charge 
                        charge=self.set_initial_charge[particle_index]*unit.elementary_charge
                    except:
                        pass
                if self.set_target_charge is not False:
                    try:
                        tgt_charge=self.set_target_charge[particle_index]*unit.elementary_charge
                    except:
                        tgt_charge=0.0*unit.elementary_charge # May exist the situation that particle_index out of the mol.pdbx atom_index, in this situation, set the target charge to 0. Only the particle_index in the alchemical_region will use the tgt_charge.
                else:
                    tgt_charge=0.0*unit.elementary_charge

                # TODO: understand how addParticle, addParticleParameterOffset, addPerParticleParameter work.
                # TODO: and make the addback of electrostatics possible.
                ...
                else: # decouple electrostatics
                    for force in all_electrostatics_custom_nonbonded_forces:
                        if particle_index in all_alchemical_atoms:
                            force.addParticle([charge, tgt_charge]) # We only change charge parameters for these in alchemical region
                        else:
                            force.addParticle([charge, charge]) # for these non-alchemical atoms, we don't change their charge parameters

                # Set offset parameters in NonbondedForce.
                if use_exact_pme_treatment and len(lambda_var_suffixes) == 1:
                    if particle_index in alchemical_atomsets[0]:
                        nonbonded_force.addParticleParameterOffset('lambda_electrostatics{}'.format(lambda_var_suffixes[0]),
                                                                   particle_index, charge-tgt_charge, 0.0, 0.0)
                    else:
                        nonbonded_force.addParticleParameterOffset('lambda_electrostatics_env',
            ...
            #     # Retrieve parameters.
            #     [charge, sigma, epsilon] = nonbonded_force.getParticleParameters(particle_index)
            #     # Even with exact treatme nt of the PME electrostatics, we turn off
            #     # the NonbondedForce charge which is modeled by the offset parameter.
                if particle_index in alchemical_atomsets[0]:
                    nonbonded_force.setParticleParameters(particle_index, tgt_charge, sigma, abs(0*epsilon))
                else:
                    nonbonded_force.setParticleParameters(particle_index, abs(0.0*charge), sigma, epsilon) # together with lambda_electrostatics_env
...
#                naa_sterics_custom_nonbonded_force.addInteractionGroup(nonalchemical_atomset, already_decoupled_alchemical_atomset)
                nat_sterics_custom_nonbonded_force.addInteractionGroup(nonalchemical_atomset, tobe_decoupled_alchemical_atomset)
                nar_sterics_custom_nonbonded_force.addInteractionGroup(nonalchemical_atomset, remaining_alchemical_atomset)
            ...
            if self.set_rbfe_exception:
                print(f'common_atomset:{common_atomset}\nunitA:{unit_A_atomset}\nunitB:{unit_B_atomset}\n')
                print(f'to be decouple:{tobe_decoupled_alchemical_atomset}')
                for interaction_group_pair in self.alchem_settings.interaction_groups:
                    aa_sterics_custom_nonbonded_force.addInteractionGroup(*interaction_group_pair)
                # pass
            else:
            ...
            if not use_exact_pme_treatment:
                ...
                if not alchemical_region.annihilate_electrostatics: #decouple
                    na_electrostatics_custom_nonbonded_force.addInteractionGroup(all_alchemical_atoms, nonalchemical_atomset)
            ...
            for exception_index in range(nonbonded_force.getNumExceptions()):
                ...
                offset_charge_prod = chargeprod

                # Calculate chargeprod and target_chargeprod
                if is_exception_chargeprod:
                    scee = 1.2
                    # logger.warning(f'We setting the scaling factor of the electrostic exception to be {scee}, which is used as default for the amber prmtop file. But you should check that carefully for your system topology file.')
                    if self.set_initial_charge is not False:
                        try:
                            iatom_ini_chg_val = self.set_initial_charge[iatom]*unit.elementary_charge
                            jatom_ini_chg_val = self.set_initial_charge[jatom]*unit.elementary_charge
                            chargeprod = iatom_ini_chg_val*jatom_ini_chg_val/scee
                        except:# May exist the situation that i or j atom index out of the mol.pdbx atom_index, in this situation, keep the chargeprod being the value in the prmtop file.
                            pass
                    # else:
                    #     chargeprod = 0.0*unit.elementary_charge*0.0*unit.elementary_charge # this could be problematic
                    if self.set_target_charge is not False:
                        try:
                            iatom_tar_chg_val = self.set_target_charge[iatom]*unit.elementary_charge
                            jatom_tar_chg_val = self.set_target_charge[jatom]*unit.elementary_charge
                            target_chargeprod = iatom_tar_chg_val*jatom_tar_chg_val/scee 
                        except:# May exist the situation that i or j atom index out of the mol.pdbx atom_index, in this situation, set the target_chargeprod to 0. 
                            target_chargeprod = 0.0*unit.elementary_charge*0.0*unit.elementary_charge
                    else:
                        target_chargeprod = 0.0*unit.elementary_charge*0.0*unit.elementary_charge

                    if alchemical_region.annihilate_electrostatics:
                        exception_charge_prod = target_chargeprod # initial charge_prod used in setExceptionParameters 
                        offset_charge_prod = chargeprod-target_chargeprod # offset used in addExceptionParameterOffset
                    else:
                        exception_charge_prod = chargeprod # initial charge_prod used in setExceptionParameters method
                        offset_charge_prod = 0.0*unit.elementary_charge*0.0*unit.elementary_charge # offset used in addExceptionParameterOffset
                else:
                    exception_charge_prod = abs(0.0*chargeprod)
                ...
                if len(lambda_var_suffixes) > 1:
                    ...
                    # added by zli: alchemical type, aa, at, ar
                    if only_one_alchemical and (iatom in already_decoupled_alchemical_atomset or jatom in already_decoupled_alchemical_atomset):
                        alchemical_type='aa'
                    if only_one_alchemical and (iatom in tobe_decoupled_alchemical_atomset or jatom in tobe_decoupled_alchemical_atomset):
                        alchemical_type='at'
                    if only_one_alchemical and (iatom in remaining_alchemical_atomset or jatom in remaining_alchemical_atomset):
                        alchemical_type='ar'
                    ...
                    if use_exact_pme_treatment and is_exception_chargeprod:
                        # modified by rdliu
                        # # added by rdliu.
                        # scee = 1.2
                        # logger.warning(f'We setting the scaling factor of the electrostic exception to be {scee}, which is used as default for the amber prmtop file. But you should check that carefully for your system topology file.')
                        ...
                        #     chargeprod = iatom_ini_chg_val*jatom_ini_chg_val/scee
                        # else:
                        #     chargeprod = 0.0*unit.elementary_charge*0.0*unit.elementary_charge
                        # if self.set_target_charge is not False:
                        #     iatom_tar_chg_val = self.set_target_charge[iatom]*unit.elementary_charge
                        #     jatom_tar_chg_val = self.set_target_charge[jatom]*unit.elementary_charge
                        #     target_chargeprod = iatom_tar_chg_val*jatom_tar_chg_val/scee 
                        ...
                        if at_least_one_alchemical:
                            nonbonded_force.addExceptionParameterOffset('lambda_electrostatics_chgprod_square{}'.format(lambda_var_suffixes[0]),
                                                                        exception_index, offset_charge_prod, 0.0, 0.0)
                            # nonbonded_force.addExceptionParameterOffset('lambda_electrostatics{}'.format(lambda_var_suffixes[0]),
                                                                        # exception_index, offset_charge_prod, 0.0, 0.0)
                        else:
                            nonbonded_force.addExceptionParameterOffset('lambda_electrostatics_env',
                ...
                # and non-alchemical particles using a single custom bond.
                    ...
                    if is_exception_epsilon:
                        if alchemical_type == 'aa':
                            naa_sterics_custom_bond_force.addBond(iatom, jatom, [sigma, epsilon])
                        if alchemical_type == 'at':
                            nat_sterics_custom_bond_force.addBond(iatom, jatom, [sigma, epsilon])
                        if alchemical_type == 'ar':
                            nar_sterics_custom_bond_force.addBond(iatom, jatom, [sigma, epsilon])
                ...
                if at_least_one_alchemical:
                    ...
                    #     target_chargeprod = 0.0*unit.elementary_charge*0.0*unit.elementary_charge
                    # nonbonded_force.setExceptionParameters(exception_index, iatom, jatom, 
                                                        #    exception_charge_prod, sigma, abs(0.0*epsilon)) # prev: 20240523
                    nonbonded_force.setExceptionParameters(exception_index, iatom, jatom,
                                                           abs(0.0*chargeprod), sigma, abs(0.0*epsilon))
                else:
                    nonbonded_force.setExceptionParameters(exception_index, iatom, jatom,
                                                           abs(0.0*chargeprod), sigma, epsilon) # together with lambda_electrostatics_env
