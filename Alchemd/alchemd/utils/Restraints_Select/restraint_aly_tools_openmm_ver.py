...
class Restrain_Data():
    ...
    def data_init(self, ):
        ...
        self.dist_init = self.dist_ary[0]
        self.ang1_init = self.ang1_ary[0]
        self.ang2_init = self.ang2_ary[0]
        self.tor1_init = self.tor1_ary[0]
        self.tor2_init = self.tor2_ary[0]
        self.tor3_init = self.tor3_ary[0]
    ...
    def calcene(self, p=None, if_mean=False, if_init_pose=False): 
        ...
        if p is None: 
            ...
            if if_init_pose:
                self.ene_list = self.cal_harm_ene(self.dist_init*10, self.dist_ary*10) + self.cal_harm_ene(self.ang1_init,self.ang1_ary) + self.cal_harm_ene(self.ang2_init,self.ang2_ary) + self.cal_harm_ene(self.tor1_init, self.tor1_ary, dihedral=True) + self.cal_harm_ene(self.tor2_init, self.tor2_ary, dihedral=True) + self.cal_harm_ene(self.tor3_init, self.tor3_ary, dihedral=True) 
        else:
    ...
    def opt_eqP(self, opt_cost_name='RED_E_cost'):
        ...
        if opt_cost_name == 'RED_E_cost':
            ...
            print('use RED_E_cost for the eq value')
        elif opt_cost_name == 'dG_forward':
            self.opt_eqP_dG_forward()
            print('use dG_forward for the eq value')
        else:
    ...
    def analyze(self, if_mean, if_init_pose):
        '''To the self.calcene() and self.fit()
        '''
        if if_mean:
            self.calcene(p=None, if_mean=True, if_init_pose=False)
            self.fit()
            self.eqP = np.array([self.dist_mean, self.ang1_mean, self.ang2_mean, self.tor1_mean, self.tor2_mean, self.tor3_mean])
        if if_init_pose:
            self.calcene(p=None, if_mean=False, if_init_pose=True)
            self.fit()
            self.eqP = np.array([self.dist_init, self.ang1_init, self.ang2_init, self.tor1_init, self.tor2_init, self.tor3_init])
...
class ALL_Restrain_Data():
    ...
    def process_data(self,  opt_cost_name=None, if_mean=False, if_init_pose=False):
        ...
        for restrain_data in self.restrain_data_list:
            ...
            if opt_cost_name is not None:
                restrain_data.opt_eqP(opt_cost_name)
            else:
                if if_mean:
                    print('use mean for the eqvalue')
                    restrain_data.analyze(if_mean, if_init_pose)
                elif if_init_pose:
                    print('use init_pose for the eq value')
                    restrain_data.analyze(if_mean, if_init_pose)
    ...
    def sort_result(self, ):
        ...
        self.restrain_data_list = [i for i in self.restrain_data_list if i.dist_ary.mean()*10 < 10] # to remove the restrain with large distance (10 Angstrom)
        self.restrain_data_list = [i for i in self.restrain_data_list if i.ang1_ary.mean() < 2.35] # to remove the restrain with large angle (135 degree)
        self.restrain_data_list = [i for i in self.restrain_data_list if i.ang2_ary.mean() < 2.35] # to remove the restrain with large angle (135 degree)
    ...
    def generate_csv(self, csv_name, lst):
        ...
        self.all_data = pd.concat([pd.DataFrame({'restraint_atom[lig1, lig2, lig3, rec1, rec2, rec3](start from 1)': [i.restrain_group,], 
                                            ...
                                            'distance between lig1 and rec1(unit: Angstrom)': [i.eqP[0]*10],
                                            'angle between lig1 rec1 and rec2(unit:radian)': [i.eqP[1]],
                                            'angle between lig2 lig1 and rec1(unit:radian)': [i.eqP[2]],
                                            'dihedral between lig1 rec1 rec2 and rec3(unit:radian)': [i.eqP[3]],
                                            'dihedral between lig2 lig1 rec1 and rec2(unit:radian)': [i.eqP[4]], 
                                            'dihedral between lig3 lig2 lig1 and rec1(unit:radian)': [i.eqP[5]], 
...
if __name__ == '__main__':
    ...
    res.process_data(opt_cost_name = 'RED_E_cost', if_mean = False, if_init_pose = False)
    ...
    lst_d = eval(open('./based_Huggins', 'r').read())
